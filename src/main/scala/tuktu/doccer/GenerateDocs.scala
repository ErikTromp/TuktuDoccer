...
object GenerateDocs {
    ...
    def readParameter(json: JsObject, writer: BufferedWriter, nestDepth: Int): Unit =  {
        ...
        writer.write(
                ...
                "* **" + name + "** *(type: " + dataType + ")* `[" + {if (required.getOrElse(false)) "Required" else "Optional"} + "]`\r\n" +
                {description match {
                    case None => "\r\n"
                    case Some(descr) => {
                        (for (i <- 1 to nestDepth * 2) yield " ").mkString("") +
                        "- " + description.getOrElse("No description present.") + "\r\n \r\n"
                    }
                }}
        )
    ...
    def readTransformer(json: JsObject, writer: BufferedWriter): Unit = {
        ...
        parameters match {
            case None => writer.write("No parameters.\r\n\r\n")
